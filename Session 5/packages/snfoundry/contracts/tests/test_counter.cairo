...
fn test_reset_counter_insufficient_allowance() {
    ...
    set_balance(caller, 100000000000000000, Token::STRK);
...
fn test_reset_counter_success() {
    ...
    set_balance(user, strk_to_fri(1), Token::STRK);
    ...
    erc_20.approve(counter.contract_address, 500000000000000000);
    ...
    let expected_event: CounterChanged = CounterChanged {
        old_value: init_counter, new_value: 0, caller: user, reason: ChangeReason::Reset,
    };

    spy.assert_emitted(@array![(counter.contract_address, Event::CounterChanged(expected_event))]);

    assert!(erc_20.balance_of(user) == 900000000000000000, "Balance not deducted correctly");
    assert!(
        erc_20.balance_of(owner_address()) == 100000000000000000, "Balance not increased correctly",
    );
}
